...
gj.grid.methods = {
    ...
    filter: function ($grid) {
        ...
        if (data.params[data.paramNames.sortBy]) {
            ...
            records.sort(
                column.sortable.sorter ? column.sortable.sorter(column.direction, column) : gj.grid.methods.createDefaultSorter(column.direction, column.field)
            );
        }
...
gj.grid.plugins.inlineEditing.private = {
    localization: function (data) {
        if (data.uiLibrary === "bootstrap") {
            ...
            data.inlineEditing.editButton =
                '<button role="edit" class="gj-button-md"><i class="gj-icon pencil" /> ' + gj.grid.messages[data.locale].Edit.toUpperCase() + "</button>";
            data.inlineEditing.deleteButton =
                '<button role="delete" class="gj-button-md"><i class="gj-icon delete" /> ' + gj.grid.messages[data.locale].Delete.toUpperCase() + "</button>";
            data.inlineEditing.updateButton =
                '<button role="update" class="gj-button-md"><i class="gj-icon check-circle" /> ' +
                gj.grid.messages[data.locale].Update.toUpperCase() +
                "</button>";
            data.inlineEditing.cancelButton =
                '<button role="cancel" class="gj-button-md"><i class="gj-icon cancel" /> ' + gj.grid.messages[data.locale].Cancel.toUpperCase() + "</button>";
        }
    },
...
gj.grid.plugins.pagination = {
    ...
    private: {
        ...
        localizationBootstrap: function (data) {
            ...
            if (typeof data.pager.leftControls === "undefined") {
                data.pager.leftControls = [
                    $('<button type="button" class="btn btn-default btn-sm">' + (data.icons.first || msg.First) + "</button>")
                        .attr("title", msg.FirstPageTooltip)
                        .attr("data-role", "page-first"),
                    $('<button type="button" class="btn btn-default btn-sm">' + (data.icons.previous || msg.Previous) + "</button>")
                    ...
                    $('<button type="button" class="btn btn-default btn-sm">' + (data.icons.next || msg.Next) + "</button>")
                        .attr("title", msg.NextPageTooltip)
                        .attr("data-role", "page-next"),
                    $('<button type="button" class="btn btn-default btn-sm">' + (data.icons.last || msg.Last) + "</button>")
                        .attr("title", msg.LastPageTooltip)
                        .attr("data-role", "page-last"),
                    $('<button type="button" class="btn btn-default btn-sm">' + (data.icons.refresh || msg.Refresh) + "</button>")
        ...
        localizationBootstrap4: function (data) {
            ...
            if (typeof data.pager.leftControls === "undefined") {
                data.pager.leftControls = [
                    $('<button class="btn btn-default btn-sm gj-cursor-pointer">' + (data.icons.first || msg.First) + "</button>")
                        .attr("title", msg.FirstPageTooltip)
                        .attr("data-role", "page-first"),
                    $('<button class="btn btn-default btn-sm gj-cursor-pointer">' + (data.icons.previous || msg.Previous) + "</button>")
                        .attr("title", msg.PreviousPageTooltip)
                        .attr("data-role", "page-previous"),
                    $("<div>" + msg.Page + "</div>"),
                    ...
                    $("<div>" + msg.Of + "</div>"),
                    $('<div data-role="page-label-last">0</div>'),
                    $('<button class="btn btn-default btn-sm gj-cursor-pointer">' + (data.icons.next || msg.Next) + "</button>")
                        .attr("title", msg.NextPageTooltip)
                        .attr("data-role", "page-next"),
                    $('<button class="btn btn-default btn-sm gj-cursor-pointer">' + (data.icons.last || msg.Last) + "</button>")
                        .attr("title", msg.LastPageTooltip)
                        .attr("data-role", "page-last"),
                    $('<button class="btn btn-default btn-sm gj-cursor-pointer">' + (data.icons.refresh || msg.Refresh) + "</button>")
                        .attr("title", msg.Refresh)
                        .attr("data-role", "page-refresh"),
                    $('<select data-role="page-size" class="form-control input-sm" width="60"></select>'),
                ];
        ...
        localizationMaterialDesign: function (data) {
            ...
            if (typeof data.pager.rightControls === "undefined") {
                data.pager.rightControls = [
                    ...
                    $('<span class="gj-md-spacer-32">&nbsp;</span>'),
                    $('<button class="gj-button-md">' + (data.icons.previous || msg.Previous) + "</button>")
                        .attr("title", msg.PreviousPageTooltip)
                        .attr("data-role", "page-previous")
                        .addClass(data.icons.first ? "gj-button-md-icon" : ""),
                    $('<span class="gj-md-spacer-24">&nbsp;</span>'),
                    $('<button class="gj-button-md">' + (data.icons.next || msg.Next) + "</button>")
                        .attr("title", msg.NextPageTooltip)
                        .attr("data-role", "page-next")
                        .addClass(data.icons.first ? "gj-button-md-icon" : ""),
                ];
        ...
        createDragStopHandler: function ($grid, $thSource) {
            return function (e, mousePosition) {
                ...
                $thSource.siblings("th").each(function () {
                    ...
                    $thTarget
                        .closest("table")
                        .find('tbody tr[data-role="row"] td:nth-child(' + ($thTarget.index() + 1) + ")")
        ...
        init: function ($grid) {
            ...
            $grid.on("rowDataBound", function (e, $row, id, record) {
                if (previousValue !== record[data.grouping.groupBy] || $row[0].rowIndex === 1) {
                    ...
                    $groupRow.append(
                        '<td colspan="' + colspan + '"><div data-role="display">' + data.grouping.groupBy + ": " + record[data.grouping.groupBy] + "</div></td>"
                    );
                        ...
                        if ($targetWrapper.droppable("isOver", mousePosition)) {
                            ...
                            if (
                                ...
                                $tree
                                    ...
                                    [data.childrenField].splice($targetNode.index() + (prepend ? 0 : 1), 0, record);
                                //END
                ...
                if ($.isArray(data.disableDates)) {
                    for (i = 0; i < data.disableDates.length; i++) {
                        if (data.disableDates[i] instanceof Date && data.disableDates[i].getTime() === date.getTime()) {
                            ...
                            gj.core.parseDate(data.disableDates[i], data.format, data.locale).getTime() === date.getTime()
                        ) {
                            result = false;
                        }
                    }
...
gj.timepicker.methods = {
    ...
    createPicker: function ($timepicker) {
        ...
        if (data.header) {
            ...
            if (data.mode === "ampm") {
                $mode.append(
                    $('<span role="am">' + gj.core.messages[data.locale].am + "</span>").on("click", function () {
                ...
                $mode.append("<br />");
                $mode.append(
                    $('<span role="pm">' + gj.core.messages[data.locale].pm + "</span>").on("click", function () {
                        var hour = gj.timepicker.methods.getHour($clock);
                        $clock.attr("mode", "pm");
                        $(this).addClass("selected");
...
gj.datetimepicker.methods = {
    ...
    initialize: function ($datetimepicker) {
        ...
        $time.append(
            $('<div role="hour" />')
                .on("click", gj.datetimepicker.methods.createShowHourHandler($datetimepicker, $picker, data))
                .html(gj.core.formatDate(new Date(), "HH", data.locale))
        );
        $time.append(":");
        $time.append(
            $('<div role="minute" />')
                .on("click", gj.datetimepicker.methods.createShowMinuteHandler($datetimepicker, $picker, data))
...
gj.slider.methods = {
    ...
    initialize: function ($slider, data) {
        ...
        gj.documentManager.subscribeForEvent(
            "mousemove",
            $slider.data("guid"),
            gj.slider.methods.createMouseMoveHandler($slider, track, handle, progress, data)
        );
