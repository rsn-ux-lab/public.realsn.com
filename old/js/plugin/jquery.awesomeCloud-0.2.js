...
(function ($) {
	...
	AwesomeCloud.prototype = {
		create: function () {
			...
			this.container.children().each(function (i, e) {
				...
				if ($(this).has("a").length === 0) {
					...
					tag = aElement.html();
					seq = aElement.attr("seq");
					//color = 
				}

				var textColor = ( this.style.color ) ? this.style.color : null;

				$this.words.push([ tag , currentWeight, link, target, textColor, seq]);
			});
			...
			window.setImmediate(function loop() {
					...
					$this.putWord($this.words[ i ][ 0 ], $this.words[ i ][ 1 ], $this.words[ i ][ 2 ], $this.words[ i ][ 3 ], $this.words[ i ][ 4 ], $this.words[ i ][ 5 ],i);
					i += 1;
			...
			canvasDOM.addEventListener("click", function (ev) {
				if ($this.match !== null) {
					if ($this.match.target) {
						...
						if( $this.settings.options.clickEvt ) {
							$this.settings.options.clickEvt( $this.match );
						}
					}
				}
			}, false);
		...
		putWord: function (word, weight, link, target, txtColor, seq, $idx) {
			if( $idx + 1 == this.words.length ) {
				$( this.container ).animate( { "opacity" : 1 }, 300 );
				if( this.settings.options.completeEvt ) this.settings.options.completeEvt();
			}
			var $this = this,
				rotate = $idx < 10 ? false : ( Math.random() < this.settings.options.rotationRatio ),
