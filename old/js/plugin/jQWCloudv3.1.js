...
function Word(wordConfig){
	...
	if( wordConfig.seq ) this.seq=wordConfig.seq;
...
Word.prototype = {
	...
	_setSpan_Size: function(){
		...
		$(span).attr( "title", this.word + " : " + this.weight );
		if(this.padding_left==null)
			this.padding_left=0;
			
		$(span).css("padding-left",this.padding_left+"px");
		$(span).html(this.word.lengthLimit( 10 ));
		
		this.width=$(span).outerWidth()+(this.padding_left*2)+4;
	    this.height=$(span).outerHeight()+4;
...
WordCloud.prototype = {
		...
		_draw: function() {
			...
			for(var index=0; index<this.words.length; index++){
				...
				if( currWord.seq ) wordConfigObj['seq']=currWord.seq;
		...
		_placeFirstWord: function(word) {	
				...
				if( word.seq ) $(span).attr( "data-seq", word.seq );
		 ...
		 _placeOtherWord: function(word) {	
				...
				for(var index=0;index<this.spaceIdArray.length;index++){
					...
					if(word.height<=obj.height){
						alignmentInd=HR;
						alignmentIndCount++;
					}	

					console.log( alignmentIndCount + "  ::  " + word.word );
