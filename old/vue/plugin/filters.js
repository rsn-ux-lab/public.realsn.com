...
Vue.filter( "lengthLimit", function( $val, $limit ){
    return $val.slice( 0, $limit ? $limit : $val.length ) + ( $limit && $limit < $val.length ? "..." : "");
});

// 소수점 반올릭
Vue.filter( "roundFixed", function( $val, $len ){
    if( !$len ) $len = 1;
    return +( Math.round( $val + "e+" + $len )  + "e-" + $len );
});
...
Vue.filter( "dateToStr", function( $val, $format, $sep ){
    ...
    if( typeof( $val ) == "string" ) {
        ...
        hour = $val.split( " " ).length > 1 && $val.split( " " )[ 1 ].split( ":" )[ 0 ] ? $val.split( " " )[ 1 ].split( ":" )[ 0 ] : "00";
        min = $val.split( " " ).length > 1 && $val.split( " " )[ 1 ].split( ":" )[ 1 ] ? $val.split( " " )[ 1 ].split( ":" )[ 1 ] : "00";
        sec = $val.split( " " ).length > 1 && $val.split( " " )[ 1 ].split( ":" )[ 2 ] ? $val.split( " " )[ 1 ].split( ":" )[ 2 ] : "00";
    } else {
...
// 두 날짜의 차이
Vue.filter( "getDateGap", function( $val, $val2 ){
    var curDate = new Date( $val );
    var targetDate = new Date( $val2 );
    return Math.ceil(Math.abs(targetDate.getTime() - curDate.getTime()) / (1000 * 3600 * 24)) + 1;
...
// 분 > 시:분으로 변환
Vue.filter( "timeConversion", function( $val ){
    var result = Math.floor( $val / 60 ) + ":" + Vue.filter( 'numAddZero' )( $val % 60 );
    return result;
});
...
// Array Merge
Vue.filter( "deepMerge", function( target, source ){    // 2개만 Merge할 경우
    Object.keys(source).filter( function( key ){
        if (source[key] instanceof Object) Object.assign(source[key], Vue.filter( 'deepMerge' )( target[key], source[key] ) );
    });
    Object.assign(target || {}, source);
    return target;
});
Vue.filter( "arrMerge", function(){			// 여러개인 경우
    ...
    args.filter( function( $arr, $idx ){
        if( $idx == 0 ) return;
        result = Vue.filter( 'deepMerge' )( result, $arr );
    });
    return result;
});
...
Vue.filter( "getParseDatas", function( $val, $key, $returnNullCheck ){
    ...
    $val.filter( function( $item ){
        ...
        if( keyCnt == sameKeyCnt ) result.push( $item );
    });
    if( !$returnNullCheck ) {
        return result;
    } else {
