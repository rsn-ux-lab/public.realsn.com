(function(){
    ...
    var template = ''
        ...
        + '            </transition>'
        + '            <transition name="fade_posy">'
        + '                <div v-if="expanded" class="wrap" :class="{ \'is-multiple\' : multiple }" :style="\'min-width:\' + getWid + \'px; max-height:\' + max_hgt + \'px\'" @scroll="evt_listScroll" v-init>'
        + '                    <div v-if="!multiple" ref="tmp1" class="not_multiple">'
        + '                        <ul :style="\'padding-top:\' + topSpace + \'px;padding-bottom:\' + bottomSpace + \'px\'">'
        ...
        + '                                    <ul>'
        + '                                        <li v-for="( item, idx ) in getCheckedLists" v-bind:key="\'checked_list_\' + idx">'
        ...
        + '                                    <ul :style="\'padding-top:\' + topSpace + \'px;padding-bottom:\' + bottomSpace + \'px\'">'
        + '                                        <li v-for="( item, idx ) in getUnCheckedLists" v-bind:key="\'unchecked_list_\' + idx">'
        + '                                            <button class="item" :value="item.code" :class="selected.indexOf( item.code ) >= 0 ? \'is-active\' : \'\'" :disabled="item.disabled" @click="evt_itemClick">{{ item.name }}</button>'
        + '                                        </li>'
        + '                                    </ul>'
        + '                                </div>'
    ...
    comp = Vue.component( "comp-selectbox", {
		...
		methods: {
            ...
            evt_listScroll: function( $e ){
                this.visibleIdxS = Math.max( 0, Math.floor( ( $e.target.scrollTop ) / 23 ) );
