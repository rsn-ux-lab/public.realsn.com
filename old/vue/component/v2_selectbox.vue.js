(function(){
    ...
    var template = ''
        ...
        + '            </transition>'
        + '            <transition name="fade_posy">'
        + '                <div v-if="expanded" class="wrap" :class="{ \'is-multiple\' : multiple }" :style="\'min-width:\' + getWid + \'px; max-height:\' + max_hgt + \'px\'" v-init>'
        + '                    <div v-if="!multiple" class="not_multiple">'
        + '                        <ul>'
        + '                            <li v-for="item in getUseOpts">'
    ...
    comp = Vue.component( "comp-selectbox", {
		...
		props: [ "id", "value", "defaultValue", "opts", "multiple", "disabled", "notSelectedLabel", "allSelectedLabel" ],
		computed: {
            getUseOpts: function(){
                var result = this.opts.concat();
                if( this.allSelectedLabel && this.opts.length > 1 ) {
                    var allCode = "";
			...
			selected_txt: function(){
				...
				if( !this.multiple ) {
                    ...
                    if( !result ) {
                        if( this.allSelectedLabel && this.opts.length > 1 ) result = this.allSelectedLabel;
