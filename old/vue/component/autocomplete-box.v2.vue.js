(function(){
    ...
    var template = ''
        ...
        + '        <button class="select_arrows" @keyup="evt_keyup" @keydown="evt_keydown" @click="evt_btnClick" :disabled="disabled" tabIndex="-1"><span>열기/닫기</span></button>'
        + '    </div>'
        + '    <div class="list" :style="\'top: \' + pos_top + \'px; left:\' + pos_left + \'px;\'">'
        + '        <transition name="fade_posy">'
        + '            <div v-if="expanded" ref="expandWrap" class="inner_wrap" @scroll="evt_listScroll" style="overflow-x:visible" :style="\'min-width:\' + getWid + \'px; max-height:\' + max_hgt + \'px; height:\' + Math.max( 30, ( getUseOpts.length * 23 ) + 8 ) + \'px\'" v-init>'
        + '                <ul v-if="!multiple || type == \'input\'" ref="list" :style="\'height:\' + ( filterDatas.length * 23 ) + \'px\'">'
        + '                    <template v-for="( item, idx ) in getUseOpts">'
        // + '                        <li v-bind:key="id + \'_\' + idx" style="position: absolute;" :style="\'top:\' + ( ( visibleIdxS + idx ) * 23 ) + \'px\'">'
        // + '                            {{ item.name }}'
        // + '                        </li>'
        + '                        <li v-bind:key="id + \'_\' + idx" style="position: absolute; width: 100%; overflow: hidden" :style="\'top:\' + ( ( visibleIdxS + idx ) * 23 ) + \'px\'">'
        // + '                            <button v-if="type == \'input\'" class="item" :class="{ \'is-active\' : idx == selectedIdx }" :data-idx="idx" @click="evt_itemClick( $event, item )" :data-data="JSON.stringify( item )" style="overflow: hidden; text-overflow: ellipsis" :title="item" tabIndex="-1">'
        + '                            <button v-if="type == \'input\'" class="item" :class="{ \'is-active\' : idx == selectedIdx }" :data-idx="idx" @click="evt_itemClick( $event, item )" :data-data="JSON.stringify( item )" style="overflow: hidden; text-overflow: ellipsis" :title="item" tabIndex="-1" v-html="String( item ).replace( inputTxt, \'<strong>\' + inputTxt + \'</strong>\' )">'
        ...
        // + '                                {{ item }}'
        + '                            </button>'
        // + '                            <button v-else class="item" :class="{ \'is-active\' : idx == selectedIdx }" :data-idx="idx" @click="evt_itemClick( $event, item )" :data-data="JSON.stringify( item )" style="overflow: hidden; text-overflow: ellipsis" :title="item.name" tabIndex="-1">'
        + '                            <button v-else class="item" :class="{ \'is-active\' : idx == selectedIdx }" :data-idx="idx" @click="evt_itemClick( $event, item )" :data-data="JSON.stringify( item )" style="overflow: hidden; text-overflow: ellipsis" :title="item.name" tabIndex="-1" v-html="String( item.name ).replace( inputTxt, \'<strong>\' + inputTxt + \'</strong>\' )">'
        ...
        + '                        <ul :style="\'height:\' + ( filterDatas.length * 23 ) + \'px\'">'
        + '                            <template v-for="( item, idx ) in getUnCheckedLists">'
        + '                                <li v-bind:key="\'unchecked_list_\' + idx" style="position: absolute; width: 100%; overflow: hidden" :style="\'top:\' + ( ( visibleIdxS + idx ) * 23 ) + \'px\'">'
        // + '                                    <button class="item" :value="item.code" :data-idx="idx" @click="evt_itemClick( $event, item )" :data-data="JSON.stringify( item )" style="overflow: hidden; text-overflow: ellipsis" :title="item" tabIndex="-1" v-html="String( item ).replace( inputTxt, \'<strong>\' + inputTxt + \'</strong>\' )">'
        ...
        + '        </transition>'
        + '    </div>'
        // + '    <div style="position: absolute; top: 0; left: 0; padding: 5px 20px; background: #ffffff; z-index: 10">{{ visibleIdxS }} :: {{ visibleIdxE }}</div>'
        + '</div>'
    ...
    var comp = Vue.component( "comp-autocomplete-box", {
            ...
            value: function( $val ){
                ...
                if( this.type == "input" ) {
                    ...
                    if( !this.multiple ) {
                        ...
                        if( !$val ) {
                            _this.selectedItem = _this.multiple ? [] : null;
                            _this.input_txt = "";
                        }
                    } else {
		...
		methods: {
            ...
            evt_listScroll: function( $e ){
                this.visibleIdxS = Math.max( 0, ( Math.floor( ( this.$refs.expandWrap.scrollTop - ( $( this.$refs.checkList ).length > 0 ? $( this.$refs.checkList ).outerHeight() : 0 ) ) / 23 ) ) - 40 );
                this.visibleIdxE = this.visibleIdxS + 90;
                // this.evt_padding();
            },
